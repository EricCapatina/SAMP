...
//============================
static pPickupID[MAX_PLAYERS char];
new MySQL:dbHandle;
...
//new hospitaltimer;
//new spraytag_timer;
new usedrugsbuff[MAX_PLAYERS char];
new Hospitalized[MAX_PLAYERS char];
new timerwork[MAX_PLAYERS char];
//============================
...
new gFractionName[FRACTION_COUNT][33] = {"", "Grove Street Families", "Front Yard Ballas", "Varrio Los Aztecas", "San Fierro Rifa", "Los Santos Vagos", "Seville B.L.V.D Families", "Rollin Heights Ballas", "Temple Drive Ballas", "Kilo Tray Ballas", "Da Nang Boys", "Sindacco", "San Fierro Triads", "LSPD", "SFPD", "LVPD", "Los Santos Army", "San Fierro Army", "Las Venturas Army", "LS Government", "SF Government", "LV Government", "Forelli"};
////////////////////
new Text:speedbox;
...
//////////////TEMP CARS
new CheckPointLS[MAX_PLAYERS char];
////////////////////
enum GZINFO
{
 	idm,
...
new
	...
	Text:Textdraw4,
	Text:ScoreCapt;
new allowedfactions[] = { 1, 2, 3, 4, 5, 6, 7, 8, 9};
new aallowedfactions[] = { 16, 17, 18 };
new OnZONE[MAX_GZONE][50];
new AZONE[MAX_AZONE][50];
...
public OnGameModeInit()
{
	...
	mysql_tquery(dbHandle, !"SELECT * FROM `armyzone`", "LoadAZ");
...
///////////////////NPC
	CreateActor(6, 452.3184, 515.5523, 1001.4195, 140.2582); // Emmet
////////////////////////MAP ICONS///////////////////////
...
////////////////////////////Kito Tray interior
	...
	///////////////////////////////// LS CHANGES
	CreateObject(1283,1821.0000000,-1748.2000000,15.6000000,0.0000000,0.0000000,180.0000000); //object(mtraffic1) (1)
	CreateObject(1283,1842.6000000,-1751.8000000,15.6000000,0.0000000,0.0000000,90.0000000); //object(mtraffic1) (2)
	CreateObject(1226,2181.0000000,-1736.3000000,16.4000000,0.0000000,0.0000000,180.0000000); //object(lamppost3) (1)
	CreateObject(3661,2135.8999000,-1738.6000000,16.0000000,0.0000000,0.0000000,270.0000000); //object(projects01_lax) (1)
	CreateObject(1501,2132.3999000,-1742.4000000,12.6000000,0.0000000,0.0000000,0.0000000); //object(gen_doorext04) (1)
...
*/
//	gruzavtols[2] = AddStaticVehicleEx(428,1639.2000000,-1840.7000000,13.8000000,0.0000000,38,55,15); //Securicar
...
stock LoadPickups()
{
	...
	blueberrypickup = CreateDynamicPickup(19605, 14, 626.8998, -596.4714, 16.6687, 0);
	govpickup = CreateDynamicPickup(19605, 14, 1614.2065, -1891.9269, 13.5469, 0);
...
stock FracPayDay()
{
    ...
    for (new stores; stores < gangstore; stores++)
	{
		...
		{
		    case 1: moneyGSF = tagsGSF*105 + gangzonesGSF*105;
		    case 2: moneyFYB = tagsFYB*105 + gangzonesFYB*105;
		    case 3: moneyAztec = tagsAztec*105 + gangzonesAztec*105;
		    case 4: moneyRifa = tagsRifa*105 + gangzonesRifa*105;
		    case 5: moneyLSV = tagsLSV*105 + gangzonesLSV*105;
		    case 6: moneySeville = tagsSeville*105 + gangzonesSeville*105;
		    case 7: moneyRHB = tagsRHB*105 + gangzonesRHB*105;
		    case 8: moneyTDB = tagsTDB*105 + gangzonesTDB*105;
		    case 9: moneyKTB = tagsKTB*105 + gangzonesKTB*105;
		}
		foreach(new i: Player)
		{
			if(player_info[i][FRACTION] == fracstorage[stores][FRACID])
			{
				...
				{
				    case 1: fracstorage[stores][FRACMONEY] += moneyGSF, format(string, sizeof(string), ftext,moneyGSF, fracstorage[stores][FRACMONEY], tagsGSF*105, gangzonesGSF*105);
				    case 2: fracstorage[stores][FRACMONEY] += moneyFYB, format(string, sizeof(string), ftext,moneyFYB, fracstorage[stores][FRACMONEY], tagsFYB*105, gangzonesFYB*105);
				    case 3: fracstorage[stores][FRACMONEY] += moneyAztec, format(string, sizeof(string), ftext,moneyAztec, fracstorage[stores][FRACMONEY], tagsAztec*105, gangzonesAztec*105);
				    case 4: fracstorage[stores][FRACMONEY] += moneyRifa, format(string, sizeof(string), ftext,moneyRifa, fracstorage[stores][FRACMONEY], tagsRifa*105, gangzonesRifa*105);
				    case 5: fracstorage[stores][FRACMONEY] += moneyLSV, format(string, sizeof(string), ftext,moneyLSV, fracstorage[stores][FRACMONEY], tagsLSV*105, gangzonesLSV*105);
				    case 6: fracstorage[stores][FRACMONEY] += moneySeville, format(string, sizeof(string), ftext,moneySeville, fracstorage[stores][FRACMONEY], tagsSeville*105, gangzonesSeville*105);
				    case 7: fracstorage[stores][FRACMONEY] += moneyRHB, format(string, sizeof(string), ftext,moneyRHB, fracstorage[stores][FRACMONEY], tagsRHB*105, gangzonesRHB*105);
				    case 8: fracstorage[stores][FRACMONEY] += moneyTDB, format(string, sizeof(string), ftext,moneyTDB, fracstorage[stores][FRACMONEY], tagsTDB*105, gangzonesTDB*105);
				    case 9: fracstorage[stores][FRACMONEY] += moneyKTB, format(string, sizeof(string), ftext,moneyKTB, fracstorage[stores][FRACMONEY], tagsKTB*105, gangzonesKTB*105);
				}
...
stock ArmyPayDay()
{
    new testek, testek2, testek3;
    new testek4, testek5, testek6;
    new testek7, testek8, testek9;
    foreach(new i: Player)
	{
		for (new az; az < ARMY_ZONE1; az++)
		{
			switch(AZMZ[az][aVillage])
		  	{
			    case 1:
			    {
			        switch(AZMZ[az][aFrak])
				    {
						case 16: testek++;
						case 17: testek2++;
						case 18: testek3++;
		...
		for (new stores; stores < gangstore; stores++)
		{
		    switch(fracstorage[stores][FRACID])
		    {
		        case 19: fracstorage[stores][FRACTOTALAZ] = testek + testek4 + testek7;
		        case 20: fracstorage[stores][FRACTOTALAZ] = testek2 + testek5 + testek8;
		        case 21: fracstorage[stores][FRACTOTALAZ] = testek3 + testek6 + testek9;
		    }
		    save_fracstores(stores);
		}
		if(testek < 10 || testek4 < 8 || testek7 < 8)
	    {
	        new stringer[160];
	        if(player_info[i][FRACTION] == 16)
			{
			    format(stringer, sizeof(stringer), "%d/10 territories - Blueberry Village", testek);
			    SCM(i, COLOR_LIGHTBLUE, stringer);
			    format(stringer, sizeof(stringer), "%d/8 territories - Dillimore Village", testek4);
			    SCM(i, COLOR_LIGHTBLUE, stringer);
			    format(stringer, sizeof(stringer), "%d/8 territories - Palomino Creek Village", testek7);
			    SCM(i, COLOR_LIGHTBLUE, stringer);
			}
	    }
	    if(testek2 < 10 || testek5 < 8 || testek8 < 8)
	    {
	        new stringer2[160];
	        if(player_info[i][FRACTION] == 17)
			{
			    format(stringer2, sizeof(stringer2), "%d/10 territories - Blueberry Village", testek2);
			    SCM(i, COLOR_LIGHTBLUE, stringer2);
			    format(stringer2, sizeof(stringer2), "%d/8 territories - Dillimore Village", testek5);
			    SCM(i, COLOR_LIGHTBLUE, stringer2);
			    format(stringer2, sizeof(stringer2), "%d/8 territories - Palomino Creek Village", testek8);
			    SCM(i, COLOR_LIGHTBLUE, stringer2);
			}
	    }
        if(testek3 < 10 || testek6 < 8 || testek9 < 8)
	    {
	        new stringer3[160];
	        if(player_info[i][FRACTION] == 18)
			{
			    format(stringer3, sizeof(stringer3), "%d/10 territories - Blueberry Village", testek3);
			    SCM(i, COLOR_LIGHTBLUE, stringer3);
			    format(stringer3, sizeof(stringer3), "%d/8 territories - Dillimore Village", testek6);
			    SCM(i, COLOR_LIGHTBLUE, stringer3);
			    format(stringer3, sizeof(stringer3), "%d/8 territories - Palomino Creek Village", testek9);
			    SCM(i, COLOR_LIGHTBLUE, stringer3);
			}
	    }
    }
}
forward TimerWorkGruz(playerid);
public TimerWorkGruz(playerid)
{
	SendClientMessage(playerid, COLOR_LIGHTBLUE, "Tuda ego");
	fraccar_info[GetPlayerVehicleID(playerid)][FRACCARBLUEBERRY] = 1;
	save_govcar(GetPlayerVehicleID(playerid));
    SetPlayerCheckpoint(playerid, 1614.2065, -1891.9269, 13.5469, 4.0);
    TogglePlayerControllable(playerid, 1);
    KillTimer(timerwork{playerid});
	return 1;
}
...
/////////////////////////////////////////LS CHANGES
	RemoveBuildingForPlayer(playerid,1283, 1816.2834, -1741.2422, 14.6762, 11.08576);
	RemoveBuildingForPlayer(playerid,1283, 1832.5002, -1747.1125, 15.42401, 11.08576);
	RemoveBuildingForPlayer(playerid,1226, 2166.8938, -1747.0914, 14.85753, 6.6149273);
	RemoveBuildingForPlayer(playerid,5551, 2140.5156, -1735.1406, 15.89063, 40.036636);
	RemoveBuildingForPlayer(playerid,1308, 2175.3125, -1730.8906, 12.70313, 8.5308437);
...
public OnPlayerSpawn(playerid)
{
	...
	SetCameraBehindPlayer(playerid);
 	...
 	else { SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]); }
 	if(IsAArmy(playerid))
	{
	    for(new k; k < ARMY_ZONE1; k++)
	    {
			GangZoneShowForPlayer(playerid, AZMZ[k][aCreate], GetArmyZoneColor(k));
		}
        for(new v; v < MAX_GANGZONE; v++)
        {
			GangZoneHideForPlayer(playerid, GZMZ[v][gCreate]);
		}
	}
	else
	{
	    for(new k; k < ARMY_ZONE1; k++)
			GangZoneHideForPlayer(playerid, AZMZ[k][aCreate]);
	}
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	if(newstate == PLAYER_STATE_DRIVER)
	{
		...
		if(fraccar_info[GetPlayerVehicleID(playerid)][FRACCARNUMBER] == 19)
	    {
	        switch(player_info[playerid][FRACTION])
	        {
	            case 19: SPD(playerid, DLG_LSRAZVOZ, DIALOG_STYLE_MSGBOX, !"Los Santos Government", !"Select village", !"Select", !"Exit");
	            default: SCMError(playerid, "Car is assigned to Los Santos Government"), RemovePlayerFromVehicle(playerid);
	        }
	    }
	    if(fraccar_info[GetPlayerVehicleID(playerid)][FRACCARNUMBER] == 5)
	    {
	        if(player_info[playerid][FRACTION] != 5)
	        {
	    ...
	    {
	        if(player_info[playerid][FRACTION] != 4)
	        {
	    ...
	    {
	        if(player_info[playerid][FRACTION] != 1)
	        {
	    ...
	    {
	        if(player_info[playerid][FRACTION] != 2)
	        {
	    ...
	    {
	        if(player_info[playerid][FRACTION] != 3)
	        {
	    ...
	    {
	        if(player_info[playerid][FRACTION] != 6)
	        {
	    ...
	    {
	        if(player_info[playerid][FRACTION] != 7)
	        {
	    ...
	    {
	        if(player_info[playerid][FRACTION] != 8)
	        {
	    ...
	    {
	        if(player_info[playerid][FRACTION] != 9)
	        {
...
public OnPlayerPickUpDynamicPickup(playerid, STREAMER_TAG_PICKUP pickupid)
{
	...
	if(pPickupID{playerid} == blueberrypickup)
	{
	    if(!(16 <= player_info[playerid][FRACTION] <= 19)) return SCMError(playerid, "You dont have permission");
	    SCM(playerid, COLOR_LIGHTBLUE, "Идет загрузка древесины, ожидайте...");
	    TogglePlayerControllable(playerid, 0);
	    timerwork{playerid} = SetTimerEx("TimerWorkGruz", 4000, 0, "d", playerid);
	    fraccar_info[GetPlayerVehicleID(playerid)][FRACCARBLUEBERRY] = 1;
	    save_govcar(GetPlayerVehicleID(playerid));
	    
	}
	if(pPickupID{playerid} == govpickup)
	{
	    if(!(player_info[playerid][FRACTION] == 19)) return SCMError(playerid, "You dont have permission");
	    if(fraccar_info[GetPlayerVehicleID(playerid)][FRACCARBLUEBERRY] == 1)
	    {
			SCM(playerid, COLOR_LIGHTBLUE, "Вы привезли материал, машина отобрана для разгрузки. Заработок казны государства:  /  личный заработок: ");
		    player_info[playerid][MONEY] += 420;
		    fraccar_info[GetPlayerVehicleID(playerid)][FRACCARBLUEBERRY] = 0;
		    save_govcar(GetPlayerVehicleID(playerid));
	...
	switch(dialogid)
	{
	    ...
	    case DLG_REGEMAIL:
	    {
	        ...
	        case DLG_REGSEX:
	        {
	            ...
	            switch(listitem)
	            {
	                case 0: player_info[playerid][SEX] = 1;
	                case 1: player_info[playerid][SEX] = 2;
	            }
	        ...
	        case DLG_LSRAZVOZ:
	        {
	            if(response)
	            {
	                SPD(playerid, DLG_LSRAZVOZSELECT, DIALOG_STYLE_TABLIST_HEADERS, !"LS Government",
					!"Village\tMaterials\tPrice\n\
					Blueberry\tWood\t{"#cBL"}$10000\n\
					Dillimore\tIron\t{"#cBL"}$17790\n\
					Palomino Creek\tDetails\t{"#cBL"}$22660", !"Select", !"Exit");
	            }
	            else SPD(playerid, -1, 0, " ", " ", " ", ""), RemovePlayerFromVehicle(playerid);
	        }
	        case DLG_LSRAZVOZSELECT:
	        {
	            for (new stores; stores < gangstore; stores++)
	            switch(listitem)
				{
				    case 0:
				    {
				        switch(fracstorage[stores][FRACID])
				        {
				            case 19:
				            {
				                if(fracstorage[stores][FRACTOTALAZ] < 5) SCM(playerid, COLOR_RED, "Your state doesn't has enough authority");
				                else CheckPointLS{playerid} = SetPlayerCheckpoint(playerid, 626.8998, -596.4714, 16.6687, 4.0);
				            }
				            case 20:
				            {
				                if(fracstorage[stores][FRACTOTALAZ] < 5) SCM(playerid, COLOR_RED, "Your state doesn't has enough authority");
				       ...
				       // CheckPointLS{playerid} = CreateDynamicCP(626.8998, -596.4714, 16.6687, 2.0, 0, 0, playerid, 15.0);
				    }
					default: SCM(playerid, COLOR_LIGHTBLUE, "Text");
				}
	        }
	        case DLG_GGPS:
	        {
			...
			if(response)
			{
				...
				for(new i; i < ARMY_ZONE1; i++)
				{
					if(PlayerToKvadrat(playerid, AZMZ[i][aCoords1], AZMZ[i][aCoords2], AZMZ[i][aCoords3], AZMZ[i][aCoords4]) && AZoneOnBattle[i] == 0)
					{
						if(AZMZ[i][aFrak] == faction) return SendClientMessage(playerid, COLOR_GREY, "Вы не можете захватывать зону вашей банды!");
						if(IsPlayerInBandOnline(faction) < 0) return SCM(playerid,COLOR_GREY, "В вашей банде мало игроков онлайн!");
						if(IsPlayerInBandOnline(AZMZ[i][aFrak]) < 0) return SCM(playerid,COLOR_GREY, "У банды мало игроков!");
						if(IsCapture == 1) return SendClientMessage(playerid, COLOR_GREY, "Уже происходит захват одной из зон. Дождитесь окончания!");
						GangZoneFlashForAll(AZMZ[i][aCreate],GetGZColorF(faction));
						gangstartid = playerid;
						captureID = i;
						AZSafeTime[i] = 60;
						FrakCD  = 3600;
						AZMZ[i][aNapad] = faction;
						ZoneOnBattle[i] = 1;
						IsCapture = 1;
						zGangTime[faction]--;
						GangInfo[faction][capture] = 1;
						GangInfo[AZMZ[i][aFrak]][capture] = 1;
						GangInfo[AZMZ[i][aFrak]][captureid] = faction;
						GangInfo[faction][captureid] = faction;
						GangInfo[faction][gangnumber] = 1;
						GangInfo[AZMZ[i][aFrak]][gangnumber] = 0;
						CaptureStart(faction, AZMZ[i][aFrak]);
						return 1;
					}
				}
			}
...
public OnPlayerTakeDamage(playerid, issuerid, Float: amount, weaponid, bodypart)
{
    ...
    switch(weaponid)
    {
        case 22:
        {
            ...
            if(usedrugsbuff[playerid] != 0 && armour > 0)
            {
...
public OnPlayerGiveDamage(playerid, damagedid, Float: amount, weaponid, bodypart)
{
    ...
    switch(weaponid)
    {
        case 0:
        {
            if(usedrugsbuff[playerid] != 0)
            {
                SetPlayerHealth(damagedid, health - 24);
            }
            if(armour > 0)
            {
                SetPlayerArmour(damagedid, armour - 7);
            }
            if(usedrugsbuff[playerid] != 0 && armour > 0)
            {
                SetPlayerArmour(playerid, armour - 11);
            }
        }
    }
	return 1;
...
CMD:help(playerid)
{
    ...
    SCM(playerid, WHITE_COLOR, "Admin Commands: /repairvehpl /setgangtag /setsex");
...
CMD:setsex(playerid, params[])
{
    ...
    if(sscanf(params, "dd", params[0], params[1])) return SCM(playerid, RED_COLOR, !"Type /setskin ID 1-2");
    if(GetPVarInt(params[0], "logged") != 1) return SCM(playerid, RED_COLOR, !"Player is not logged in");
	switch(params[1])
	{
	    case 1: player_info[params[0]][SEX] = 1;
	    case 2: player_info[params[0]][SEX] = 2;
	}
	static const fmt_query[] = "UPDATE `users` SET `sex` = '%d' WHERE `id` = '%d' LIMIT 1";
	new query[sizeof(fmt_query)+(-2+24)+(-2+11)];
	mysql_format(dbHandle, query, sizeof(query), fmt_query, player_info[params[0]][SEX], player_info[params[0]][ID]);
	mysql_query(dbHandle, query);
	return 1;
}
CMD:bandana(playerid, params[])
{
	if(!IsAGang(playerid)) return SCMError(playerid, "You're not a gang member");
	if(sscanf(params, "d", params[0])) return SCM(playerid, RED_COLOR, !"Type /bandana 0-1");
	if(params[0] == 1)
	{
	    if(player_info[playerid][SEX] == 1)
	    {
			switch(player_info[playerid][FRACTION])
			{
			    case 1: SetPlayerAttachedObject(playerid, 1, 18913, 2, 0.1, 0.03, -0.005, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]); // Females skin :SetPlayerAttachedObject(playerid, 1, 18918, 2, 0.07, 0.03, -0.005, 90.0, 180.0, 90.0, 0xFFFF0000);
			    case 2: SetPlayerAttachedObject(playerid, 1, 18915, 2, 0.1, 0.03, -0.005, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]);
			    case 3: SetPlayerAttachedObject(playerid, 1, 18909, 2, 0.123, 0.01, 0.0, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]);// Females skin    SetPlayerAttachedObject(playerid, 1, 18909, 2, 0.1, 0.01, 0.0, 90.0, 180.0, 90.0);
			    case 4: SetPlayerAttachedObject(playerid, 1, 18908, 2, 0.123, 0.01, 0.0, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]);
			    case 5: SetPlayerAttachedObject(playerid, 1, 18916, 2, 0.1, 0.03, -0.005, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]);
			    case 6: SetPlayerAttachedObject(playerid, 1, 18891, 2, 0.123, 0.01, 0.0, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]);
			    case 7: SetPlayerAttachedObject(playerid, 1, 18899, 2, 0.123, 0.01, 0.0, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]);
			    case 8: SetPlayerAttachedObject(playerid, 1, 18897, 2, 0.123, 0.01, 0.0, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]);
			    case 9: SetPlayerAttachedObject(playerid, 1, 18917, 2, 0.1, 0.03, -0.005, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]);
			}
		}
		else if(player_info[playerid][SEX] == 2)
	    {
			switch(player_info[playerid][FRACTION])
			{
			    case 1: SetPlayerAttachedObject(playerid, 1, 18913, 2, 0.07, 0.03, -0.005, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]); // Females skin :SetPlayerAttachedObject(playerid, 1, 18918, 2, 0.07, 0.03, -0.005, 90.0, 180.0, 90.0, 0xFFFF0000);
			    case 2: SetPlayerAttachedObject(playerid, 1, 18915, 2, 0.07, 0.03, -0.005, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]);
			    case 3: SetPlayerAttachedObject(playerid, 1, 18909, 2, 0.1, 0.01, 0.0, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]);// Females skin    SetPlayerAttachedObject(playerid, 1, 18909, 2, 0.1, 0.01, 0.0, 90.0, 180.0, 90.0);
			    case 4: SetPlayerAttachedObject(playerid, 1, 18908, 2, 0.1, 0.01, 0.0, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]);
			    case 5: SetPlayerAttachedObject(playerid, 1, 18916, 2, 0.07, 0.03, -0.005, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]);
			    case 6: SetPlayerAttachedObject(playerid, 1, 18891, 2, 0.1, 0.01, 0.0, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]);
			    case 7: SetPlayerAttachedObject(playerid, 1, 18899, 2, 0.1, 0.01, 0.0, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]);
			    case 8: SetPlayerAttachedObject(playerid, 1, 18897, 2, 0.1, 0.01, 0.0, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]);
			    case 9: SetPlayerAttachedObject(playerid, 1, 18917, 2, 0.07, 0.03, -0.005, 90.0, 180.0, 90.0), SetPlayerColor(playerid, gFractionColor[player_info[playerid][FRACTION]]);
			}
		}
	}
	else
...
CMD:makeleader(playerid, params[])
{
    ...
    if(!(0 <= params[1] <= 22)) return SCM(playerid, RED_COLOR, !"Leader id must be 0 - 22");
    if(GetPVarInt(params[0], "logged") != 1) return SCM(playerid, RED_COLOR, !"Player is not logged in");
...
public LoadGZ()
{
	...
	return printf("> [MySQL] Loading Zones: Loaded - %d", MAX_GANGZONE + ARMY_ZONE1);
}
forward LoadAZ();
public LoadAZ()
{
    ...
    for (new gz; gz < ARMY_ZONE1; gz++)
	{
    	cache_get_value_name_int(gz, "id", AZMZ[gz][ida]);
		cache_get_value_name_float(gz, "ainfo1", AZMZ[gz][aCoords1]);
		cache_get_value_name_float(gz, "ainfo2", AZMZ[gz][aCoords2]);
		cache_get_value_name_float(gz, "ainfo3", AZMZ[gz][aCoords3]);
		cache_get_value_name_float(gz, "ainfo4", AZMZ[gz][aCoords4]);
		cache_get_value_name_int(gz, "fraction", AZMZ[gz][aFrak]);
		cache_get_value_name_int(gz, "village", AZMZ[gz][aVillage]);

		AZMZ[gz][aCreate] = GangZoneCreate(AZMZ[gz][aCoords1], AZMZ[gz][aCoords2], AZMZ[gz][aCoords3], AZMZ[gz][aCoords4]);
	}
	return 1;
}
forward LoadGTags();
...
public LoadFracStore()
{
    ...
    for (new stores; stores < gangstore; stores++)
	{
		...
		cache_get_value_name_int(stores, "wood", fracstorage[stores][FRACWOOD]);
		cache_get_value_name_int(stores, "iron", fracstorage[stores][FRACIRON]);
		cache_get_value_name_int(stores, "details", fracstorage[stores][FRACDETAILS]);
		cache_get_value_name_int(stores, "totalgz", fracstorage[stores][FRACTOTALGZ]);
		cache_get_value_name_int(stores, "totalaz", fracstorage[stores][FRACTOTALAZ]);
	}
...
public LoadFRACCAR()
{
    ...
    for (new cars; cars < MAXFRACCAR; cars++)
	{
		...
		cache_get_value_name_int(cars, "blueberry", fraccar_info[x][FRACCARBLUEBERRY]);
		cache_get_value_name_int(cars, "dilimore", fraccar_info[x][FRACCARDILLIMORE]);
		cache_get_value_name_int(cars, "palomino", fraccar_info[x][FRACCARPALOMINO]);
...
stock save_govcar(goid)
{
    new query[512];
	format(query, sizeof(query), "UPDATE `fraccar` SET `blueberry` = '%d', `dilimore` = '%d', `palomino` = '%d' WHERE `fraction` = '%d'",
	fraccar_info[goid][FRACCARBLUEBERRY], fraccar_info[goid][FRACCARDILLIMORE], fraccar_info[goid][FRACCARPALOMINO], fraccar_info[goid][FRACCARFRACTION]);
	mysql_tquery(dbHandle, query);
}
...
{
	new query[512];
	format(query, sizeof(query), "UPDATE `fracstore` SET `money` = '%d', `drugs` = '%d', `ammo` = '%d', `wood` = '%d', `iron` = '%d', `details` = '%d', `totalgz` = '%d', `totalaz` = '%d' WHERE `fraction` = '%d'",
	fracstorage[fsid][FRACMONEY], fracstorage[fsid][FRACDRUGS], fracstorage[fsid][FRACAMMO], fracstorage[fsid][FRACWOOD], fracstorage[fsid][FRACIRON], fracstorage[fsid][FRACDETAILS], fracstorage[fsid][FRACTOTALGZ], fracstorage[fsid][FRACTOTALAZ], fracstorage[fsid][FRACID]);
	mysql_tquery(dbHandle, query);
	return 1;
}
...
{
	new query[138 + 24 + 24 + 24 + 24 + 4 + 3];
	format(query, sizeof(query), "UPDATE `armyzone` SET `ainfo1` = '%f', `ainfo2` = '%f', `ainfo3` = '%f', `ainfo4` = '%f', `fraction` = '%d' WHERE `id` = '%d' LIMIT 1",
	AZMZ[aid][aCoords1], AZMZ[aid][aCoords2],AZMZ[aid][aCoords3],AZMZ[aid][aCoords4],AZMZ[aid][aFrak], aid+1);
	mysql_tquery(dbHandle, query);
	return 1;
}
...
stock GetArmyZoneColor(armyzonex)
{
	new zx;
	switch(AZMZ[armyzonex][aFrak])
	{
		case ARMY_LS: zx = 0x009900AA;
		case ARMY_SF: zx = 0xffff00AA;
		case ARMY_LV: zx = 0x6666FFAA;
		case 0: zx = 0xC0C0C0AA;
	}
	return zx;
}
...
stock GetGZColorF(fnumber)
{
	...
	switch(fnumber)
	{
		...
		case F_TDBALLAS: zx = 0xEE84FF90;
		case F_KTBALLAS: zx = 0x9F00FF55;
		case 16: zx = 0x009900AA;
		case 17: zx = 0xffff00AA;
...
stock GetGangName(fnumbwer)
{
	...
	switch(fnumbwer)
	{
		...
		case 16: format(string,128,"Los Santos");
		case 17: format(string,128,"San Fierro");
		case 18: format(string,128,"Las Venturas");
	}
	return string;
}
stock CaptureStart(familyone, familytwo)
{
	...
	foreach(new i:Player)
	{
		...
		SCMA( CRED, mes);
		format(mes, sizeof(mes), "%s начал захват территории %s", player_info[gangstartid][NAME], GetGangName(familytwo));
...
stock IsInAallowed(playerid)
{
	new L = sizeof(aallowedfactions);
	for(new i =0;i<L;i++)
	{
		if(player_info[playerid][FRACTION] == aallowedfactions[i]) return 1;
	}
	return 0;
}
...
CMD:acapture(playerid)
{
	...
	if(IsAArmy(playerid))
	{
		...
		//if(p_info[playerid][pRank] < 9) return SendClientMessage(playerid,WHITE_COLOR,"Функция недоступна");
		new faction = player_info[playerid][FRACTION];
		if(!IsInAallowed(playerid)) return SCMError(playerid, "Ваша банда уже учавствует в захвате зон");
		if(zGangTime[faction] <= 0) return SCMError(playerid, "Сегодня ваша банда больше не может учавствовать в захвате");
		if(FrakCD > 0)
		{
		    new f_str[43];
		...
		for(new i; i< ARMY_ZONE1;i++)
		{
			if(PlayerToKvadrat(playerid, AZMZ[i][aCoords1], AZMZ[i][aCoords2], AZMZ[i][aCoords3], AZMZ[i][aCoords4]))
			{
			    if(ZoneOnBattle[i] == 0)
			    {
				    if(AZMZ[i][aFrak] == player_info[playerid][FRACTION]) return SendClientMessage(playerid, WHITE_COLOR, "Вы не можете захватывать зону вашей банды!");
					//if(IsPlayerInBandOnline(faction) < 5) return SCMError(playerid, "В вашей банде мало игроков онлайн!");
					//if(IsPlayerInBandOnline(GZMZ[i][gFrak]) < 5) return SCMError(playerid, "У банды мало игроков!");
					if(IsCapture == 1) return SendClientMessage(playerid, WHITE_COLOR, "Уже происходит захват одной из зон. Дождитесь окончания!");
					format(string, sizeof(string), "Эта территория принадлежит %s\nВы уверены, что хотите захватить ее?",GetGangName(GZMZ[i][gFrak]));
					SPD(playerid,DLG_ACAPTURE,0,"{F3FF02}Захват территории",string,"Да","Нет");
				}
			}
		}
	}
...
stock AzCheck()
{
	for(new i; i < ARMY_ZONE1; i++)
	{
		if(ZoneOnBattle[i] == 1 && AZSafeTime[i] == 0)
		{
			if(GangInfo[AZMZ[i][aNapad]][score] > GangInfo[AZMZ[i][aFrak]][score] ||
			GangInfo[AZMZ[i][aNapad]][score] == 0 && GangInfo[AZMZ[i][aFrak]][score] == 0)
			{
			    new string[64];
				new zx,zl;
				zx = AZMZ[i][aNapad];
				...
				CaptureEnd();
				GangInfo[AZMZ[zl][aNapad]][captureid] = 0;
				GangInfo[AZMZ[zl][aNapad]][capture] = 0;
				GangInfo[AZMZ[zl][aNapad]][score] = 0;
				GangInfo[AZMZ[zl][aFrak]][captureid] = 0;
				GangInfo[AZMZ[zl][aFrak]][capture] = 0;
				GangInfo[AZMZ[zl][aFrak]][score] = 0;
				GangInfo[AZMZ[i][aNapad]][gangnumber] = 0;
				GangInfo[AZMZ[i][aFrak]][gangnumber] = 0;
				AZMZ[zl][aFrak] = zx;
				AZMZ[zl][aNapad] = 0;
				GangZoneStopFlashForAll(AZMZ[zl][aCreate]);
				GangZoneHideForAll(AZMZ[zl][aCreate]);
				GangZoneShowForAll(AZMZ[zl][aCreate],GetGangZoneColor(zl));
				save_gz(zl);
				update_gang();
			}
			else if(GangInfo[AZMZ[i][aNapad]][score] < GangInfo[AZMZ[i][aFrak]][score] ||
			GangInfo[AZMZ[i][aNapad]][score] == GangInfo[AZMZ[i][aFrak]][score])
			{
			    new string[64];
				new zx,zl;
				zx = AZMZ[i][aFrak];
				...
				AZMZ[zl][gFrak] = zx;
				AZMZ[zl][gNapad] = 0;
				GangZoneStopFlashForAll(AZMZ[zl][gCreate]);
				GangZoneHideForAll(AZMZ[zl][aCreate]);
				GangZoneShowForAll(AZMZ[zl][aCreate],GetArmyZoneColor(zl));
				save_gz(zl);
				update_gang();
			}
		}
...
CMD:azcolor(playerid, params[])
{
    if(player_info[playerid][ADMIN] < 4) return 1;
    if(sscanf(params,"d",params[0])) return SendClientMessage(playerid, WHITE_COLOR, "/azcolor [ARMY ID]");
    if(!(16 <= params[0] <= 18)) return SCM(playerid, RED_COLOR, !"IDArmy must be 16 - 18");
 	for(new i; i <= ARMY_ZONE1; i++)
	{
		if(PlayerToKvadrat(playerid, AZMZ[i][aCoords1], AZMZ[i][aCoords2], AZMZ[i][aCoords3], AZMZ[i][aCoords4]))
		{
			AZMZ[i][aFrak] = params[0];
			GangZoneStopFlashForAll(AZMZ[i][aCreate]);
			GangZoneHideForAll(AZMZ[i][aCreate]);
			GangZoneShowForAll(AZMZ[i][aCreate], GetArmyZoneColor(i));
			save_az(i);
			if(i+1 == AZMZ[i][ida])
			{
			    new string[80];
			    format(string, sizeof(string), "You have changed %d armyzone color", AZMZ[i][ida]);
			    SCM(playerid, COLOR_LIGHTBLUE, string);
			}
			return 1;
		}
...
CMD:setskin(playerid, params[])
{
    ...
    new strings[120];
    format(strings, sizeof(strings), !"Administrator %s gave you %d Skin ID", player_info[playerid][NAME], params[1]);
	SCM(params[0], RED_COLOR, strings);
...
stock Checkarmour(playerid)
{
	...
	if(armour != 0)
	{
		...
		{
		    case 1: SetPlayerAttachedObject(playerid, 9, 19515, 1, 0.046000, 0.050999, 0.000000, 0.000000, 0.000000, 0.000000, 1.087000, 1.26, 1.189999);
		    case 2: SetPlayerAttachedObject(playerid, 9, 19515, 1, 0.046000, 0.050999, 0.000000, 0.000000, -2.900000, 0.000000, 1.150000, 1.26, 0.809999);
		}
	}
