...
#define 	SPRAYTAG_THREAD_LOAD 		(1)
#define 	SPRAYTAG_THREAD_SAVE 		(2)
...
{
	print("\n--------------------------------------");
	print(" Spray Tag System [MySQL] - English Version | Versão: 0.2b - By: Nurb Will;");
	print("--------------------------------------\n");
	spraytag_mysql = mysql_connect(SQL_HOST, SQL_USER, SQL_DB, SQL_PASS);
	return 1;
}
...
public OnPlayerConnect(playerid)
{
	new 
		...
		szQuery[80]
	;
	printf("[Spray Tag's] %s Loading spray tags.", szName);
	GetPlayerName(playerid, szName, MAX_PLAYER_NAME);
	format(szQuery, sizeof szQuery, "SELECT * FROM `spray_tags` WHERE owner = '%s'", szName);
	mysql_function_query(spraytag_mysql, szQuery, true, "SprayTag_Load", "ii", 0, SPRAYTAG_THREAD_LOAD);
	return 1;
}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch(dialogid)
	{
		...
		case SPRAYTAG_FIND:
		{
			...
			format(szMessage, sizeof szMessage, "[Spray Tag] The checkpoint was dialed to your spray tag #%i.", listitem);
			SendClientMessage(playerid, -1, szMessage);
		}
		...
		case SPRAYTAG_CREATE_MAIN:
		{
			...
			switch(listitem)
			{
				case 0: ShowPlayerDialog(playerid, SPRAYTAG_CREATE_TEXT, DIALOG_STYLE_INPUT, "{F7EC6F}Spray Tag - {469BF0}Texto", "Enter the text you would like to appear on the Spray Tag", "Done", "Back");
				case 1: ShowPlayerDialog(playerid, SPRAYTAG_CREATE_FONT, DIALOG_STYLE_LIST, "{F7EC6F}Spray Tag - {469BF0}Fonte", "Arial\nCourier\nImpact\nPricedown\nDaredevil\nBombing\naaaiight! fat\nFrom Street Art\nGhang\nGraffogie\nGraphers Blog\nNosegrind Demo", "Done", "Back");
				case 2: ShowPlayerDialog(playerid, SPRAYTAG_CREATE_SIZE, DIALOG_STYLE_INPUT, "{F7EC6F}Spray Tag - {469BF0}Tamanho", "Enter the size do you want appear on the Spray Tag", "Done", "Back");
				case 3: ShowPlayerDialog(playerid, SPRAYTAG_CREATE_COLOR, DIALOG_STYLE_LIST, "{F7EC6F}Spray Tag - {469BF0}Cor", "{FF0000}Red\n{04B404}Green\n{00B5CD}Light Blue\n{FFFF00}Yellow\n{0000FF}Blue\n{848484}Grey\n{FF00FF}Pink\n{FFFFFF}White", "Done", "Back");
				case 4: 
				{
					switch(spraytag_bold[playerid])
					{
...
public SprayTag_Load(extraid, type) {
	new rows, fields;
	cache_get_data(rows, fields, spraytag_mysql);
	switch(type)
	{
		case SPRAYTAG_THREAD_LOAD:
		{
			if(IsPlayerConnected(extraid))
			{
				...
				while( i < rows)
				{
					...
					new result[32];
					cache_get_field_content(i, "id", result, spraytag_mysql); SprayTags[extraid][i][_spSQLID] = strval(result);
					cache_get_field_content(i, "owner", SprayTags[extraid][i][_spOwned], spraytag_mysql, MAX_PLAYER_NAME);
					...
					cache_get_field_content(i, "fontsize", result, spraytag_mysql); SprayTags[extraid][i][_spFontSize] = strval(result);
					cache_get_field_content(i, "bold", result, spraytag_mysql); SprayTags[extraid][i][_spBold] = strval(result);
					cache_get_field_content(i, "color", result, spraytag_mysql); SprayTags[extraid][i][_spFontColor] = strval(result);
					cache_get_field_content(i, "posx", result, spraytag_mysql); SprayTags[extraid][i][_spPosX] = floatstr(result);
					cache_get_field_content(i, "posy", result, spraytag_mysql); SprayTags[extraid][i][_spPosY] = floatstr(result);
					cache_get_field_content(i, "posz", result, spraytag_mysql); SprayTags[extraid][i][_spPosZ] = floatstr(result);
					cache_get_field_content(i, "posrx", result, spraytag_mysql); SprayTags[extraid][i][_spPosRX] = floatstr(result);
					cache_get_field_content(i, "posry", result, spraytag_mysql); SprayTags[extraid][i][_spPosRY] = floatstr(result);
					cache_get_field_content(i, "posrz", result, spraytag_mysql); SprayTags[extraid][i][_spPosRZ] = floatstr(result);
					cache_get_field_content(i, "vw", result, spraytag_mysql); SprayTags[extraid][i][_spVW] = strval(result);
					cache_get_field_content(i, "interior", result, spraytag_mysql); SprayTags[extraid][i][_spInt] = strval(result);
...
forward SprayTag_QueryCreate(playerid, iIndex);
public SprayTag_QueryCreate(playerid, iIndex)
{
	SprayTags[playerid][iIndex][_spSQLID] = cache_insert_id(spraytag_mysql);
	
	GetPlayerName(playerid, SprayTags[playerid][iIndex][_spOwned], MAX_PLAYER_NAME);
...
stock SprayTag_Create(playerid, iIndex)
{
	if(IsPlayerConnected(playerid))
	{
		...
		;
		GetPlayerName(playerid, szName, MAX_PLAYER_NAME);
		format(szQuery, sizeof(szQuery), "INSERT INTO `spray_tags` (owner, text, font, fontsize, bold, color, posx, posy, posz, posrx, posry, posrz, vw, interior) VALUES ('%s', '%s', '%s', '%d', '%d', '%d', '%f', '%f', '%f', '%f', '%f', '%f', '%d', '%d')",
		szName,
		SprayTags[playerid][iIndex][_spText],
		SprayTags[playerid][iIndex][_spFont],
		SprayTags[playerid][iIndex][_spFontSize],
		SprayTags[playerid][iIndex][_spBold],
		SprayTags[playerid][iIndex][_spFontColor],
		SprayTags[playerid][iIndex][_spPosX],
		SprayTags[playerid][iIndex][_spPosY],
		SprayTags[playerid][iIndex][_spPosZ],
		SprayTags[playerid][iIndex][_spPosRX],
		SprayTags[playerid][iIndex][_spPosRY],
		SprayTags[playerid][iIndex][_spPosRZ],
		SprayTags[playerid][iIndex][_spVW],
		SprayTags[playerid][iIndex][_spInt]
		);

		mysql_function_query(spraytag_mysql, szQuery, true, "SprayTag_QueryCreate", "ii", playerid, iIndex);
	}
	return 1;
}
...
stock SprayTag_Save(playerid, iIndex)
{
	if(IsPlayerConnected(playerid))
	{
		new query[400];
		format(query, sizeof(query), "UPDATE `spray_tags` SET \
			`owner` = '%s', \
			`text` = '%s', \
			`font` = '%s', \
			`fontsize` = %d, \
			...
			`posrz` = %f, \
			`vw` = %d, \
			`interior` = %d WHERE `id` = %d",
			SprayTags[playerid][iIndex][_spOwned],
			SprayTags[playerid][iIndex][_spText],
			SprayTags[playerid][iIndex][_spFont],
			SprayTags[playerid][iIndex][_spFontSize],
			SprayTags[playerid][iIndex][_spBold],
			...
			SprayTags[playerid][iIndex][_spVW],
			SprayTags[playerid][iIndex][_spInt],
			SprayTags[playerid][iIndex][_spSQLID]
		);
		mysql_function_query(spraytag_mysql, query, false, "SprayTag_Load", "ii", iIndex, SPRAYTAG_THREAD_SAVE);
	}
	return 1;
}
...
{
	new szQuery[80];
	format(szQuery, sizeof(szQuery), "DELETE FROM `spray_tags` WHERE `id` = '%d'", SprayTags[playerid][iIndex][_spSQLID]);
	mysql_function_query(spraytag_mysql, szQuery, false, "SprayTag_Load", "ii", iIndex, SPRAYTAG_THREAD_SAVE);
	
	format(SprayTags[playerid][iIndex][_spOwned], 32, "Ninguem");
	format(SprayTags[playerid][iIndex][_spText], 32, "Exemplo");
	format(SprayTags[playerid][iIndex][_spFont], 32, "Arial");
